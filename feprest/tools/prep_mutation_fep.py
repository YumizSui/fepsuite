...
class LigandProteinMutationGenerator(DefaultProteinMutationGenerator):
    """Protein mutation FEP generator with a non-mutating ligand present.

    The ligand is parameterized (or pre-prepared ITP is used), and combined
    with the protein after fepgen generates the mutation topology. The ligand
    does not change between states A and B.
    """

    # ligand parameters
    ligand_info: dict  # output from ligand_param.parameterize_ligand or use_existing_itp

    # RNA/DNA residue names to exclude from FASPR (FASPR only handles protein)
    RNA_RESIDUES = {
        'A', 'C', 'G', 'U', 'I',
        'DA', 'DC', 'DG', 'DT', 'DI', 'DU',
        'RA', 'RC', 'RG', 'RU', 'RI',
    ...
    def __init__(self, pdb: str, gmx: str, python3: str, faspr: str, fepsuite: str,
        ...
        super().__init__(pdb, gmx, python3, faspr, fepsuite,
                         ff, water_model, solv, solv_ref,
                         ion, ion_positive, ion_negative, difficult_nrep)

        # Prepare ligand parameters
        if ligand_sdf:
            from ligand_param import parameterize_ligand
            self.ligand_info = parameterize_ligand(
                sdf_path=ligand_sdf,
                output_dir="ligand_param",
                acpype_exe=acpype_exe,
            )
        elif ligand_itp and ligand_gro:
    ...
    def _strip_non_protein(self, pdb_path: str, output_path: str):
        ...
        FASPR can only handle protein side chains. This method removes:
        - All HETATM records (ligands, waters, ions)
        - ATOM records for RNA/DNA residues

        Returns:
            List of removed lines for later restoration
        """
        stripped_lines = []
        with open(pdb_path) as fh, open(output_path, 'w') as ofh:
            for line in fh:
    ...
    def _restore_non_protein(self, pdb_path: str, non_protein_lines: List[str], output_path: str):
        """Restore non-protein lines (RNA/DNA) into FASPR output PDB.

        Args:
            pdb_path: FASPR output PDB (protein only)
            non_protein_lines: Lines removed by _strip_non_protein
            output_path: Output path for combined PDB
        """
        with open(pdb_path) as fh:
            protein_lines = fh.readlines()

        with open(output_path, 'w') as ofh:
            # Write protein ATOM/TER lines
            for line in protein_lines:
                if line.startswith(('END', 'MASTER')):
                    break
                ofh.write(line)
            # Add TER if needed, then RNA/DNA lines
            if non_protein_lines:
                # Add TER to separate protein from nucleic acid
                if protein_lines and not protein_lines[-1].startswith('TER'):
                    ofh.write('TER\n')
                for line in non_protein_lines:
                    ofh.write(line)
            ofh.write('END\n')

    def generate_gmx(self, basepdbinfo, ddir, mutations):
        """Override to strip HETATM and RNA before pdb2gmx, then restore RNA."""
        if not os.path.exists(ddir):
            os.mkdir(ddir)
        else:
            if os.path.exists(f"{ddir}/conf.pdb") and os.path.exists(f"{ddir}/topol_can.top") and os.path.exists(f"{ddir}/gmx_success"):
        ...
        # Strip HETATM and RNA/DNA from the input PDB used for FASPR
        protein_only_pdb = f"{ddir}/protein_only.pdb"
        non_protein_lines = self._strip_non_protein(self.pdb, protein_only_pdb)

        # Generate mutant using protein-only PDB
        old_pdb = self.pdb
        self.pdb = protein_only_pdb
        try:
            self.generate_mutant(basepdbinfo, ddir, mutations)
        finally:
            self.pdb = old_pdb

        # Restore RNA/DNA lines to FASPR output (completed.pdb)
        if non_protein_lines:
            completed_pdb = f"{ddir}/completed.pdb"
            completed_pdb_tmp = f"{ddir}/completed_with_rna.pdb"
            self._restore_non_protein(completed_pdb, non_protein_lines, completed_pdb_tmp)
            shutil.move(completed_pdb_tmp, completed_pdb)
            log_with_color(f"Restored {len(non_protein_lines)} non-protein lines to {completed_pdb}")
        ...
        # Canonicalize topology (same as parent)
        with open("topol.top") as ifh, open("topol_m.top", "w") as ofh:
            out = True
            for l in ifh:
                if re.search("POSRES", l):
    ...
    def _remove_ligand_from_topology(self, top_path: str):
        """Remove ligand ITP includes and molecule entries from topology.

        Used to clean reference system topologies that should not include ligands.
        Removes lines matching:
        - #include ".*_GMX.itp" or similar ligand ITP files
        - Ligand molecule entries in [ molecules ] section
        """
        ligand_name = self.ligand_info['ligand_name']
        ...
        with open(top_path, 'w') as ofh:
            ...
            for line in lines:
                ...
                # Skip ligand ITP include lines
                if '#include' in line and ligand_name in line:
                    continue

                # Track [ molecules ] section
                if tokens == ['[', 'molecules', ']']:
                    in_molecules = True
                elif stripped.startswith('[') and in_molecules:
                    in_molecules = False

                # Skip ligand molecule entry in [ molecules ]
                if in_molecules and tokens and tokens[0] == ligand_name:
                    continue

                ofh.write(line)

    def _combine_with_ligand(self, fepdirname: str):
        """Combine fepgen output with ligand structure and topology.

        After fepgen produces fepbase.pdb and fepbase.top (protein-only FEP
        topology), this method adds the ligand coordinates and ITP include.
        """
        from combine_ligand import combine_structure, combine_topology_inline

        ligand_name = self.ligand_info['ligand_name']
        ligand_itp = self.ligand_info['itp']
        ligand_gro = self.ligand_info.get('gro')
        ligand_top = self.ligand_info.get('top')

        if ligand_gro and os.path.isfile(ligand_gro):
            # Combine structure: add ligand coordinates to fepbase.pdb
            protein_pdb = os.path.join(fepdirname, "fepbase.pdb")
            combined_pdb = os.path.join(fepdirname, "fepbase_with_lig.pdb")
            combine_structure(protein_pdb, ligand_gro, combined_pdb)
            # Replace fepbase.pdb with combined version
            shutil.move(combined_pdb, protein_pdb)
            log_with_color(f"Combined ligand coordinates into {protein_pdb}")

            # Also combine fepbase_A.pdb and fepbase_B.pdb
            for suffix in ["_A", "_B"]:
                state_pdb = os.path.join(fepdirname, f"fepbase{suffix}.pdb")
                if os.path.isfile(state_pdb):
                    combined_state = os.path.join(fepdirname, f"fepbase{suffix}_with_lig.pdb")
                    combine_structure(state_pdb, ligand_gro, combined_state)
                    shutil.move(combined_state, state_pdb)

        # Combine topology: add ligand ITP include and molecule entry
        protein_top = os.path.join(fepdirname, "fepbase.top")
        combined_top = os.path.join(fepdirname, "fepbase_with_lig.top")
        combine_topology_inline(protein_top, ligand_itp, ligand_name,
                                combined_top, ligand_top=ligand_top)
        shutil.move(combined_top, protein_top)
        log_with_color(f"Combined ligand topology into {protein_top}")

        # Copy ligand ITP to the FEP directory for relative includes
        itp_dest = os.path.join(fepdirname, os.path.basename(ligand_itp))
        if not os.path.exists(itp_dest):
            shutil.copy2(ligand_itp, itp_dest)

    def generate(self, muts, muts_name, base_muts="", base_name=None, base_mut_separator="_"):
        """Override generate to add ligand combination step after fepgen."""
        basepdbinfo = parse_pdb(self.pdb)
        ...
        # Generate FEP topology (protein only, same as parent)
        fepdirname = base_mut_separator.join(fepdir)
        refdirs = [f"{fepdirname}_ref"]
        if len(mutation_list_diff) > 1:
            refdirs = []
        ...
        # Reference systems (protein-only, no ligand)
        for refdir, m in zip(refdirs, mutation_list_diff):
            other_resids = [mx.resid for mx in mutation_list_diff if mx.resid != m.resid]
            log_with_color(f"mkdir {refdir}")
            os.makedirs(refdir, exist_ok=True)
...
def argparse_options():
    ...
    # Ligand options (Feature 1: protein mutation FEP with ligand present)
    ligand_group = parser.add_argument_group("Ligand options",
        "Options for including a non-mutating ligand in the protein mutation FEP system")
    ligand_group.add_argument("--ligand-sdf", default=None, type=str,
        help="Ligand SDF file (will be parameterized with ACPYPE)")
    ligand_group.add_argument("--ligand-itp", default=None, type=str,
        help="Pre-prepared ligand ITP file (alternative to --ligand-sdf)")
    ligand_group.add_argument("--ligand-gro", default=None, type=str,
        help="Ligand GRO coordinate file (required with --ligand-itp)")
    ligand_group.add_argument("--acpype", default="acpype", type=str,
        help="Path to ACPYPE executable")
...
if __name__ == "__main__":
    ...
    if has_ligand:
        ...
        generator = DefaultProteinMutationGenerator(
            args.pdb, args.gmx, args.python3, args.faspr, args.fepsuite,
            args.ff, args.water_model,
            args.solv, args.solv_ref, args.ion, args.ion_positive, args.ion_negative,
            args.difficult_nrep,
